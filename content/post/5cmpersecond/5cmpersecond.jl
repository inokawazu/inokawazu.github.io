...
# ╔═╡ 3ea40ca2-79a3-11f0-078c-096d181c4ff3
using DifferentialEquations

# ╔═╡ cd42ef2a-1e0c-49fd-a66d-bd469783bd35
using PlutoUI

# ╔═╡ 9c02c289-7b93-49ad-a150-1ce8a35509ec
using Accessors

# ╔═╡ fa163367-e314-423c-b7e4-da1dad180eb6
using WGLMakie

# ╔═╡ d5be904e-7f4f-4063-b680-40bb69400af1
function petal_ode!(ddu, du, u, p, t)
	(; ks, g, alpha, R, kp) = p
	_, _, th = u
	dx, dy, dth = du
	rA = -sin(th) * dx + cos(th) * dy
	rrh = cos(th) * dx + sin(th) + dy
	ddu[1] = ks * (rA * sin(th) + alpha * R * dth * cos(th)/4) - kp * dx
	ddu[2] = - g + ks * (-rA * cos(th) + alpha * R * dth * sin(th)/4) - kp * dy
	ddu[3] = ks * (alpha/R * rrh - dth)
	return
end

# ╔═╡ 2d264e33-61e7-4c21-9851-bb808e99a4b5
function make_ground_cb(ground_y = 0)
	function cb_cond(u,t,integrator)
		return u[2 + 3] - ground_y
	end
	return ContinuousCallback(cb_cond, terminate!)
end

# ╔═╡ 046cacc4-546b-4566-aae6-8ba905abc653
default_p = (ks = 10.0, g = 9.81, R = 0.01, alpha = 0.03, h0 = 3.0, kp = 0.0)

# ╔═╡ 562e21ff-d7e4-4fa8-9eeb-2c120b72d100
function solve_static_start(h0, th0, p = default_p; tspan = (0.0, Inf))
	cb = make_ground_cb()
	u0 = [0, h0, th0]
	du0 = zero(u0)
	
	prob = SecondOrderODEProblem(petal_ode!, du0, u0, tspan, p)
	sol = solve(prob, RK4(), callback = cb)
end

# ╔═╡ 979085b6-9dbf-4d25-a00d-4a38a0ec6a3c
cmfig1 = let
	h0 = default_p.h0
	th0s = range(0, 2pi, length=100)
	sols = Iterators.map(th0s) do th0
		solve_static_start(h0, th0)
	end
	fig = Figure()
	ax = Axis(fig[1, 1])
	for sol in sols
		plot!(ax, sol, idxs = (4, 5))
	end
	ax.xlabel = "x (m)"
	ax.ylabel = "y (m)"
	fig
...
# ╔═╡ 9a1ad559-c2d5-4b1e-9251-254c0a7bedbe
save("cm_xy_1.png", cmfig1);

# ╔═╡ 6df991d5-3a7d-403c-bb4a-cf4895fe824d
cmfig2 = let
	p = default_p
	...
	th0s = range(0, 2pi, length=40)
	sols = Iterators.map(th0s) do th0
		solve_static_start(p.h0, th0, p)
	end
	fig = Figure()
	ax = Axis(fig[1, 1])
	for sol in sols
...
# ╔═╡ 08f0a9b7-96db-4aff-ade2-f97fc616f400
save("cm_xy_2.png", cmfig2);

# ╔═╡ b88e2105-dde6-423c-b36c-e9466d099aa9
let
	th0s = range(0, 2pi, length=20)
	p = default_p
	...
	axtime = Axis(fig[1, 1])
	plot!(axtime, [sol.u[begin][6] for sol in sols], [sol.t[end] for sol in sols])
	axtime.xlabel = "Initial Angle"
	axtime.ylabel = "Fall Time (s)"

	axspeed = Axis(fig[2, 1])
	plot!(axspeed, [sol.u[begin][6] for sol in sols], [p.h0/sol.t[end] for sol in sols])
	axspeed.xlabel = "Initial Angle"
	axspeed.ylabel = "Average Vertical Velocity (m/s)"
	fig
end

# ╔═╡ c97bd615-f172-4932-8af4-43a0fe8bc098
md"""
# Test Run
"""

# ╔═╡ ced94c98-929d-42ef-9e95-7cc941ecdbdb
@bind testth0 Slider(range(0, pi, length=100), show_value=true)

# ╔═╡ f607958d-5174-4a53-a75e-eef708c40c2c
testsol = let 
	tspan = (0.0, 10.0)
	u0 = [0.0, 20.0, testth0]
	du0 = [0.0, 0.0, 0.0]
	...
	prob = SecondOrderODEProblem(petal_ode!, du0, u0, tspan, p)
	sol = solve(prob, ImplicitEuler(), callback = cb)
end

# ╔═╡ 7c997dfa-e281-4e6a-a7b1-56b5327393be
let
	fig, ax = plot(testsol, idxs = (4, 5))
	ax.xlabel = "x"
	ax.ylabel = "y"
	fig
end

# ╔═╡ 0274beab-9270-4552-ae9f-919af1587b45
plot(testsol, idxs = (0, 6))
...
manifest_format = "2.0"
project_hash = "9500046605c8b45cde291d370fef9df812a5c1fb"
...
git-tree-sha1 = "890634c88f18511ce8383a8335475f081c97387a"
...
git-tree-sha1 = "d84e026183bb3fc764110ae98792c2e004f8c240"
...
git-tree-sha1 = "b0615008c7cf8c459a52225a3c62d9c61467f799"
...
git-tree-sha1 = "a0d5f963cfee70bee26dfa137a732e759e24436a"
