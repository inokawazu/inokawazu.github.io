...
# ╔═╡ 777a9220-60a9-11f0-2c39-f988063c6b01
using DifferentialEquations

# ╔═╡ ac6b01c1-7c6b-423c-909d-2474e1811c19
using WGLMakie

# ╔═╡ 88ef3164-98a7-4b56-91bd-a2b2b7aa9678
md"""
# Unitless Relativistic 2nd Law + Constant Force
"""

# ╔═╡ 8714beca-bd4f-4795-8b07-78b7ab9b55fa
function constantforce!(du, u, alpha, _)
	dq, q, s = u
	du[1] = alpha*(1 - dq^2)^2
	du[2] = dq
	du[3] = sqrt(1 - dq^2)
...
# ╔═╡ 8b743a0e-7690-46fe-8744-fae695c4a7bb
sol = solve(ODEProblem(constantforce!, [0.0, 0.0, 0.0], (0.0, 100.0), 3.0))

# ╔═╡ 602822b0-fada-4e95-b765-ed11c55d070b
function getdest(alpha, q = 1/2)
	@assert alpha > 0
	function terminate_callback(u, t, integrator)
		u[2] - q
	end
	prob = ODEProblem(constantforce!, [0.0, 0.0, 0.0], (0.0, Inf), alpha)
	sol = solve(prob, save_everystep=false, callback = ContinuousCallback(terminate_callback, terminate!))
	[sol.t[end]; sol[end]]
end

# ╔═╡ a96ee700-6b88-4766-be5f-5f1c26aa2f4b
fig1 = let alphas = range(0.1, 6, step = 0.01)
	
	ss = 2 * map(x->getdest(x)[4], alphas)
	ts = 2* map(x->getdest(x)[1], alphas)

	lines(alphas, ss, label = L"\sigma")
	lines!(alphas, ts, label = L"t")
	hlines!([1.0], label = L"t_{light}", color = :black, linestyle=:dash)
	current_axis().xlabel = L"\alpha"
	ylims!(current_axis(), 0, 4)
	xlims!(current_axis(), 0, 6)
	axislegend()
	current_figure()
end

# ╔═╡ 032183aa-8725-4590-8826-56a0d6678168
fig2 = let amax = 20, alphas = range(0.1, amax, step = 0.01)
	
	ss = 2 * map(x->getdest(x, 1/2)[4], alphas)
	ts = 2 * map(x->getdest(x, 1/2)[1], alphas)
...
# ╔═╡ 8a4ef0de-c592-4cb0-9102-755fd776661e
three_times[1]/three_times[4]
...
manifest_format = "2.0"
project_hash = "e4186502122575fa3d391baac290e12153796364"
