...
# ╔═╡ dadff9ee-2217-4887-a678-29adfcf5b323
using DifferentialEquations

# ╔═╡ 7dcfa72d-bd96-4a6a-95c2-c31d42a45ffa
using WGLMakie

# ╔═╡ 32386c10-9cec-44d6-9178-2e5e2768b08f
using ForwardDiff

# ╔═╡ 4899e5f1-36f1-4b60-87d7-63472ebc135c
using Roots

# ╔═╡ 4bea567e-6539-11f0-2e25-cd9a3b8d4f94
function constantforce!(du, u, (alpha, a), t)
	dq, q, s, t = u
	at = a(t)
	dat = ForwardDiff.derivative(a, t)
	du[1]  = dat * dq * (at^2 * dq^2 - 2)/ at
	du[1] += alpha * (1 - at^2 * dq^2)^2 / at^2
	du[2] = dq
	du[3] = 1 - at^2 * dq^2
	du[3] = du[3] >= 0 ? sqrt(du[3]) : zero(du[3])
	du[4] = 1
	return
end

# ╔═╡ 4880a515-4f6a-463e-88ae-7e862a0c792c
function constantforce_turn!(du, u, (alpha, a, turntime), t)
	constantforce!(du, u, (alpha * (t < turntime ? 1 : -1), a), t)
	return
end

# ╔═╡ e3071ca9-3d91-4ff8-90e7-6ddb7679d239
linear_a(H) = t -> H * t + 1

# ╔═╡ b0b473c6-957f-4cfc-a065-c4a0fd84f485
function static_start(q0)
	dq0 = zero(q0)
	[dq0, q0, 0.0, 0.0]
end

# ╔═╡ 182d645b-792a-4070-9f09-dbf2e4667213
# function finalpos(q0, (alpha, delta), t)
# 	sol = solve(ODEProblem(constantforce!, static_start(q0), (0.0, 3.0), (0.0, 10.0)), RK4(), save_everystep=false)
# 	sol[end]
# end

# ╔═╡ 917acd5d-0f31-4ab2-95b8-3d3edebc6de7
tsol = solve(ODEProblem(constantforce!, static_start(0.0), (0.0, 10.0), (2.0, linear_a(0.2))), RK4())

# ╔═╡ 6c0ee6fa-847e-40a9-853c-0f093f2299c9
plot(tsol, idxs = (4, 2))

# ╔═╡ a9ddd9b2-5be5-4097-9a29-f86d7c19c8d0
function destsolve(; alpha = 3.0, a = linear_a(0.1), rev_time = 1.0, solve_opts = (;))
	function terminate_callback(u, t, integrator)
		t > 0 ? u[1] : one(u[1])
	end
	cb = ContinuousCallback(terminate_callback, terminate!)
	...
	sol = solve(prob, RK4(), callback = cb; solve_opts...)
end

# ╔═╡ 59347aa1-2afa-469a-865c-2f69c4d63f71
function destsolve_endpoint(; solve_opts = (save_everystep=false,), vargs...)
	 destsolve(; solve_opts = solve_opts, vargs...)[end]
end

# ╔═╡ fd369eac-2de8-4e09-a652-436b658649a2
function destsolve_full(; solve_opts = (;), vargs...)
	 destsolve(; solve_opts = solve_opts, vargs...)
end

# ╔═╡ 67920523-0472-44c3-a4e9-06cf84941b1b
plot(destsolve_full(alpha = 10.0, a = linear_a(1.1), rev_time = 10.0), idxs = [(0, 2)])

# ╔═╡ 4f3b0822-deee-4472-9a8e-5cc39072a7c6
function solve_revtime(alpha, a, L = 1.0, (min_revtime, max_revtime) = (0.0, 10.0))
	f(rt) = destsolve_endpoint(; rev_time = rt, a = a, alpha = alpha)[2] - L
	Roots.find_zero(f, (min_revtime, max_revtime))
end

# ╔═╡ 32a78a97-c2be-4d58-bf91-d1af304e5713
myalphas = range(0.2, 5.0, step = 0.01)

# ╔═╡ 5230513f-f2cd-4400-9a0b-deaaa216ac53
mya = linear_a(0.2)

# ╔═╡ c16a06e8-f6ff-408d-a9a4-074742dc4930
myrev_times = solve_revtime.(myalphas, mya)

# ╔═╡ c908ee07-79a5-4bab-933e-a189ff6fb33a
myends = map(zip(myrev_times, myalphas)) do (rev_time, alpha)
	destsolve_endpoint(; rev_time = rev_time, a = mya, alpha = alpha)
end |> stack |> permutedims

# ╔═╡ 53d01ae2-cabd-4b31-b87b-4f7124949395
lines(myalphas, myends[:, 3])
...
manifest_format = "2.0"
project_hash = "26b2346472b27dc87b85398d65c5fc4646040622"
